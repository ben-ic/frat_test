    ...
    ~H"""
      <div>
        <%= if @paid_invoices > 0  do %>
          <div class="hero">Are you sure you want to withdraw $<%= @paid_invoices %> to your personal bank account</div>
          <div class="modal-action">
            <button for="my-modal-5" class="btn btn-success" phx-click="withdraw" phx-target={@myself}>Yes</button>
            <button for="my-modal-5" class="btn btn-cancel" phx-click="cancel" phx-target={@myself}>No</button>
          </div>
        <% else %>
          <div class="hero">You have no money in your account to withdraw</div>
          <button for="my-modal-5" class="btn btn-cancel" phx-click="cancel" phx-target={@myself}>Okay</button>
        <% end %>
      </div>
    """
  end
